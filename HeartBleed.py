...
options = OptionParser(usage='%prog server [option]', description='Test for SSL heartbeat vulnerability (CVE-2014-0160)')
...
def recvmsg(s):
    hdr = recvall(s, 5)
    if hdr is None:
        print ('header - closed connection')
        return None, None, None
    typ, ver, ln = struct.unpack('>BHH', hdr)
    pay = recvall(s, ln, 10)
    if pay is None:
        print ('record payload - closed connection')
        return None, None, None
    ...
    while True:
        ...
        if typ == 24:
            ...
            if len(pay) > 3 :
                ...
                print("Did not return any extra data")
            return True
        if typ == 21:
...
# 20 : ChangeCipherSpec Protocol    21 : Alert Protocol     22 : handshake Protocol     23 : Application Protocol
def main():
